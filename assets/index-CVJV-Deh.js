...
68. **provided** /prəˈvaɪdɪd/
    conj. 如果，只要，在…条件下`,Ef=`# 2027考研英语红宝书
## 必考词Unit10 77词
...
71. **writer** /ˈraɪtə(r)/
    n. 作家，作者；写的人
`,Bf=`# 2027考研英语红宝书必考词Unit 7（80词）提取结果
...
`,Uf=/^\s*(\d+)\.\s+\*\*(.+?)\*\*\s+(\/.+\/)\s*$/,Wf=/^\s*([A-Za-z][A-Za-z'’-]*(?:\s+[A-Za-z][A-Za-z'’-]*)?)\s+(\/[^/]+\/)\s*$/,Gf=/^((?:[a-z]+\.)+(?:\s+[a-z]+\.)*)\s+(.+)$/i;function Kf(e,t){let n=(e.match(/^#{1,3}\s+(.+)$/m)?.[1]?.trim())?.match(/(必备词|必考词)\s*Unit\s*(\d+)/i);return n?`${n[1]} Unit${n[2]}`:t}function qf(e){let t=[...(e.match(/^#{1,3}\s+(.+)$/m)?.[1]||``).matchAll(/\d+/g)].map(e=>Number(e[0]));return t.length?t[t.length-1]:void 0}function Jf(e){return e.map(e=>e.trim()).filter(Boolean).map(e=>{let t=e.match(Gf);return t?{pos:t[1].trim(),meaning:t[2].trim()}:{pos:``,meaning:e}})}function Yf(e,t){let n=t.unitName||Kf(e,t.unitId),r=e.split(/\r?\n/),i=[],a,o=()=>{if(!a)return;let e=i.length+1;i.push({id:`${t.unitId}-${String(e).padStart(3,`0`)}`,unitId:t.unitId,unitName:n,order:e,word:a.word,phonetic:a.phonetic,definitions:Jf(a.definitions)})};for(let e of r){let t=e.match(Uf);if(t){o(),a={order:Number(t[1]),word:t[2].trim(),phonetic:t[3].trim(),definitions:[]};continue}let n=e.match(Wf);if(n){o(),a={order:i.length+1,word:n[1].trim(),phonetic:n[2].trim(),definitions:[]};continue}a&&(!e.trim()||e.trim().startsWith(`#`)||a.definitions.push(e))}return o(),{id:t.unitId,name:n,order:t.order,expectedCount:qf(e),words:i,referenceImages:t.referenceImages}}var Xf={radiate:`太阳向四周发光发热，就是 radiate「散发、辐射」。`,radiant:`脸上像发光一样，就是 radiant「容光焕发的、灿烂的」。`,radical:`从根上彻底改变，就是 radical「根本的、激进的」。`,object:`摆在眼前的东西是 object，也可以表示「反对」。`,objective:`不带个人情绪地看目标，就是 objective「客观的；目标」。`,objection:`别人说“不行”，这就是 objection「反对、异议」。`,obligation:`必须承担的事，就是 obligation「义务、责任」。`,oblige:`责任压过来，让你不得不做，就是 oblige「迫使」。`,obscure:`被雾遮住看不清，就是 obscure「模糊、难懂」。`,observation:`认真看并记录想法，就是 observation「观察、评论」。`,observe:`看见规则也照着做，就是 observe「观察、遵守」。`,obsession:`一件事总在脑子里转，就是 obsession「痴迷」。`,obsolete:`旧设备没人再用，就是 obsolete「过时的」。`,obtain:`努力后拿到想要的，就是 obtain「获得」。`,obvious:`一眼就看得出来，就是 obvious「明显的」。`,ideal:`心里最满意的样子，就是 ideal「理想的」。`,ideology:`一整套想法和立场，就是 ideology「意识形态」。`,identical:`两样东西完全一样，就是 identical「相同的」。`,identification:`确认身份的证明，就是 identification「识别、身份证明」。`,identify:`把人或问题认出来，就是 identify「确认、识别」。`,identity:`证明“我是谁”的东西，就是 identity「身份」。`,journal:`定期记录文章的本子，就是 journal「期刊、日志」。`,journalist:`跑新闻写报道的人，就是 journalist「记者」。`,journey:`一段从这里到那里的路，就是 journey「旅程」。`,judge:`法庭上作判断的人，就是 judge「法官、判断」。`,judgement:`对事情作出的判断，就是 judgement「看法、判决」。`,judicial:`和法庭审判有关的，就是 judicial「司法的」。`,jury:`在法庭上帮助 judge 判断的人，就是 jury「陪审团」。`,jurisdiction:`法院能管到的范围，就是 jurisdiction「管辖权」。`,justice:`法律追求公平结果，就是 justice「公正、正义」。`,justify:`给自己的说法找理由，就是 justify「证明有理」。`,label:`给东西贴上名字牌，就是 label「标签、贴标签」。`,lag:`进度慢半拍跟不上，就是 lag「落后、滞后」。`,largely:`占了大部分情况，就是 largely「主要地、大半地」。`,lateral:`从旁边横着伸出去，就是 lateral「侧面的、横向的」。`,latter:`前后两个里后面的那个，就是 latter「后者的」。`,law:`社会共同遵守的规则，就是 law「法律」。`,lawsuit:`因为纠纷打到法院去，就是 lawsuit「诉讼、官司」。`,magnitude:`大到能衡量影响，就是 magnitude「巨大、重要性」。`,magnify:`用放大镜看得更大，就是 magnify「放大、夸大」。`,magnificent:`大场面让人惊叹，就是 magnificent「宏伟的」。`,maintain:`让状态一直保持住，就是 maintain「维持、保养」。`,maintenance:`为了保持可用而照料，就是 maintenance「维护、保养」。`,major:`最重要、最大的一项，就是 major「主要的、专业」。`,majority:`人数占了大头，就是 majority「大多数」。`,make:`亲手把东西做出来，就是 make「做、制造」。`,theme:`文章反复围绕的中心，就是 theme「主题」。`,theory:`用来解释现象的一套说法，就是 theory「理论」。`,theoretical:`还停留在理论层面，就是 theoretical「理论上的」。`,therapy:`用方法帮身体或心理恢复，就是 therapy「疗法」。`,qualification:`证明你够资格的条件，就是 qualification「资格」。`,qualify:`达到条件能参加，就是 qualify「使合格、有资格」。`,quality:`东西好不好，看的就是 quality「质量、品质」。`,qualitative:`关注性质而不是数量，就是 qualitative「定性的」。`,safeguard:`把安全守住，就是 safeguard「保护、保障」。`,safety:`不会受伤出事的状态，就是 safety「安全」。`,savage:`凶狠又不讲理，就是 savage「野蛮的、凶恶的」。`,save:`把人救下或把钱留下，就是 save「拯救、节省」。`,saving:`攒下来的钱或节省，就是 saving「存款、节省」。`,scale:`用刻度看大小范围，就是 scale「规模、刻度」。`,scene:`眼前发生的一幕，就是 scene「场面、现场」。`,scenery:`旅途中看到的自然画面，就是 scenery「风景」。`,pace:`走路或做事的速度，就是 pace「步伐、节奏」。`,panel:`讨论问题的一组人，就是 panel「专门小组」。`,panorama:`一眼看到完整景象，就是 panorama「全景」。`,prove:`拿出证据说明是真的，就是 prove「证明」。`,provide:`把需要的东西给别人，就是 provide「提供」。`,provided:`有前提条件才成立，就是 provided「只要、如果」。`},Zf=Object.assign({"../../单词文本/分区/19.md":_f,"../../单词文本/分区/20.md":vf,"../../单词文本/分区/21.md":yf,"../../单词文本/分区/22.md":bf,"../../单词文本/分区/23.md":xf,"../../单词文本/分区/24.md":Sf,"../../单词文本/分区/25.md":Cf,"../../单词文本/分区/26.md":wf,"../../单词文本/分区/必修1.md":Tf,"../../单词文本/分区/必修10.md":Ef,"../../单词文本/分区/必修11.md":Df,"../../单词文本/分区/必修12.md":Of,"../../单词文本/分区/必修13.md":kf,"../../单词文本/分区/必修14.md":Af,"../../单词文本/分区/必修15.md":jf,"../../单词文本/分区/必修16.md":Mf,"../../单词文本/分区/必修17.md":Nf,"../../单词文本/分区/必修18.md":Pf,"../../单词文本/分区/必修2.md":Ff,"../../单词文本/分区/必修3.md":If,"../../单词文本/分区/必修4.md":Lf,"../../单词文本/分区/必修5.md":Rf,"../../单词文本/分区/必修6 .md":zf,"../../单词文本/分区/必修7.md":Bf,"../../单词文本/分区/必修8 .md":Vf,"../../单词文本/分区/必修9.md":Hf});function Qf(e){return(e.split(`/`).pop()||e).replace(/\.[^.]+$/,``)}function $f(e){let t=e.match(/\d+/)?.[0];if(t)return Number(t)}function ep(e,t){let n=t?.match(/Unit\s*(\d+)/i)?.[1];return n?Number(n):$f(Qf(e))||999}function tp(e){return e.id===`unit1`?{...e,words:e.words.map(e=>({...e,memoryTip:Xf[e.word.toLowerCase()]}))}:e}var np=Object.entries(Zf).map(([e,t])=>{let n=ep(e,t);return tp(Yf(t,{unitId:`unit${n}`,unitName:`必备词 Unit${n}`,order:n}))}).sort((e,t)=>e.order-t.order);np.find(e=>e.id===`unit1`)||np[0],np.find(e=>e.id===`unit2`);var rp=np.flatMap(e=>e.words),ip=`当前线上包未读取到 Supabase 配置，请先配置 GitHub Secrets 并重新部署。你仍可使用游客模式背单词。`;function ap(){let[e,t]=(0,y.useState)(null),[n,r]=(0,y.useState)(null),[i,a]=(0,y.useState)(null),[o,s]=(0,y.useState)(Fc),[c,l]=(0,y.useState)(null),u=(0,y.useCallback)(async(e=n)=>{if(!e||!Fc)return a(null),null;let t=await Fl(e.id)||await Il(e);return a(t),t},[n]);return(0,y.useEffect)(()=>{if(!Fc||!Ic){s(!1);return}let e=!0;Pl().then(async({session:n,user:i})=>{e&&(t(n),r(i),i&&await u(i))}).catch(t=>{e&&l(t instanceof Error?t.message:`读取登录状态失败`)}).finally(()=>{e&&s(!1)});let{data:n}=Ic.auth.onAuthStateChange((e,n)=>{let i=n?.user||null;t(n),r(i),i?u(i).catch(()=>void 0):a(null)});return()=>{e=!1,n.subscription.unsubscribe()}},[u]),{configured:Fc,session:e,user:n,profile:i,loading:o,error:c,login:(0,y.useCallback)(async(e,n)=>{if(!Fc)throw l(ip),Error(ip);s(!0),l(null);try{let i=await Rl({email:e,password:n});return t(i.session),r(i.user),i.user&&await u(i.user),i}catch(e){throw l(e instanceof Error?e.message:`登录失败`),e}finally{s(!1)}},[u]),register:(0,y.useCallback)(async(e,n,i)=>{if(!Fc)throw l(ip),Error(ip);s(!0),l(null);try{let a=await Ll({username:e,email:n,password:i});return t(a.session),r(a.session?a.user:null),a.session&&a.user&&await u(a.user),a}catch(e){throw l(e instanceof Error?e.message:`注册失败`),e}finally{s(!1)}},[u]),logout:(0,y.useCallback)(async()=>{s(!0),l(null);try{await zl(),t(null),r(null),a(null)}catch(e){throw l(e instanceof Error?e.message:`退出登录失败`),e}finally{s(!1)}},[]),refreshProfile:u}}var op=new Map(rp.map(e=>[e.id,e.unitId])),sp=new Set([`learned`,`known`,`fuzzy`,`forgotten`,`quiz-correct`,`quiz-wrong`,`monster-defeated`]);function cp(e){return e.lastReviewedAt||e.nextReviewAt||e.firstLearnedAt||new Date(0).toISOString()}function lp(e){return sp.has(e)?e:`known`}function up(e,t){return{user_id:e,word_id:t.wordId,unit_id:op.get(t.wordId)||null,stage:t.stage,first_learned_at:t.firstLearnedAt||null,last_reviewed_at:t.lastReviewedAt||null,next_review_at:t.nextReviewAt||null,known_count:t.knownCount,fuzzy_count:t.fuzzyCount,forgotten_count:t.forgottenCount,correct_count:t.correctCount,wrong_count:t.wrongCount,monster_hp:null,is_mastered:Cn(t),raw_progress:t,updated_at:cp(t)}}function dp(e,t){return Object.values(t).flatMap(t=>t.reviewHistory.map(n=>({user_id:e,word_id:t.wordId,unit_id:op.get(t.wordId)||null,action:n.action,old_stage:n.stage,new_stage:t.stage,created_at:n.at})))}function fp(e){let t=e.raw_progress&&typeof e.raw_progress==`object`?e.raw_progress:{};return{...$t(e.word_id),...t,wordId:e.word_id,learned:!!(t.learned??e.first_learned_at),stage:Number(e.stage||0),firstLearnedAt:e.first_learned_at||t.firstLearnedAt,lastReviewedAt:e.last_reviewed_at||t.lastReviewedAt,nextReviewAt:e.next_review_at||t.nextReviewAt,knownCount:Number(e.known_count||0),fuzzyCount:Number(e.fuzzy_count||0),forgottenCount:Number(e.forgotten_count||0),correctCount:Number(e.correct_count||0),wrongCount:Number(e.wrong_count||0),reviewHistory:Array.isArray(t.reviewHistory)?t.reviewHistory:[]}}async function pp(e,t,n){let r=Bc(),i=await r.from(e).delete().eq(`user_id`,t);if(i.error)throw i.error;if(!n.length)return;let a=await r.from(e).insert(n);if(a.error)throw a.error}async function mp(e,t){let n=Bc(),r=Object.values(t).map(t=>up(e,t));if(r.length){let e=await n.from(`word_progress`).upsert(r,{onConflict:`user_id,word_id`});if(e.error)throw e.error}await pp(`review_history`,e,dp(e,t))}async function hp(e){let t=Bc(),n=await t.from(`word_progress`).select(`*`).eq(`user_id`,e);if(n.error)throw n.error;let r={};(n.data||[]).forEach(e=>{let t=fp(e);r[t.wordId]=t});let i=await t.from(`review_history`).select(`*`).eq(`user_id`,e).order(`created_at`,{ascending:!0});if(i.error)throw i.error;let a=new Map;return(i.data||[]).forEach(e=>{let t=a.get(e.word_id)||[];t.push({action:lp(e.action),at:e.created_at,stage:Number(e.new_stage??e.old_stage??0)}),a.set(e.word_id,t)}),a.forEach((e,t)=>{r[t]={...r[t]||$t(t),reviewHistory:e.slice(-200)}}),r}async function gp(e){let{count:t,error:n}=await Bc().from(`word_progress`).select(`id`,{count:`exact`,head:!0}).eq(`user_id`,e);if(n)throw n;return!!(t&&t>0)}function _p(){return new Date().toISOString()}async function vp(e,t,n){let r=Bc(),i=await r.from(e).delete().eq(`user_id`,t);if(i.error)throw i.error;if(!n.length)return;let a=await r.from(e).insert(n);if(a.error)throw a.error}function yp(e,t){return{user_id:e,total_study_days:t.totalStudyDays,current_streak:t.currentStreak,longest_streak:t.longestStreak,total_study_minutes:t.totalStudyMinutes,total_words_learned:t.totalWordsLearned,total_words_reviewed:t.totalWordsReviewed,total_quiz_count:t.totalQuizCount,total_correct_count:t.totalCorrectCount,total_wrong_count:t.totalWrongCount,total_xp:t.totalXp,level:t.level||Kt(t.totalXp),coins:t.coins,diamonds:t.diamonds,monster_points:t.monsterPoints,defeated_monsters:t.defeatedMonsters,raw_stats:t,updated_at:_p()}}function bp(e,t){return{user_id:e,default_accent:t.audioSettings.defaultAccent,auto_play_on_study:t.audioSettings.autoPlayOnStudy,auto_play_on_recall:t.audioSettings.autoPlayOnRecall,speech_rate:t.audioSettings.speechRate,repeat_count:t.audioSettings.repeatCount,theme:t.equippedTheme,updated_at:_p()}}function xp(e,t){return t.map(t=>({user_id:e,mode:t.mode,unit_id:t.unitId||null,started_at:t.startedAt,ended_at:t.endedAt,duration_seconds:t.durationSeconds,word_count:t.wordCount,correct_count:t.correctCount??null,wrong_count:t.wrongCount??null,created_at:t.startedAt}))}function Sp(e,t){return t.map(t=>({user_id:e,date:t.date,type:`checkin`,streak_day:t.streakDay,reward_xp:t.rewardXp,reward_coins:t.rewardCoins,created_at:t.checkedAt}))}function Cp(e,t){return t.map(t=>({user_id:e,type:t.type,title:t.title,description:t.description,xp:t.xp,coins:t.coins,diamonds:0,created_at:t.createdAt}))}function wp(e,t){return Array.from(new Set(t)).map(t=>({user_id:e,achievement_id:t,unlocked_at:_p()}))}function Tp(e,t){return t.map(t=>({user_id:e,item_id:t.itemId,count:t.count,obtained_at:t.obtainedAt,updated_at:_p()}))}function $(e,t=0){let n=Number(e);return Number.isFinite(n)?n:t}function Ep(e){let t=e?.raw_stats&&typeof e.raw_stats==`object`?e.raw_stats:{};return dn({...t,totalStudyDays:$(e?.total_study_days,t.totalStudyDays),currentStreak:$(e?.current_streak,t.currentStreak),longestStreak:$(e?.longest_streak,t.longestStreak),totalStudyMinutes:$(e?.total_study_minutes,t.totalStudyMinutes),totalWordsLearned:$(e?.total_words_learned,t.totalWordsLearned),totalWordsReviewed:$(e?.total_words_reviewed,t.totalWordsReviewed),totalQuizCount:$(e?.total_quiz_count,t.totalQuizCount),totalCorrectCount:$(e?.total_correct_count,t.totalCorrectCount),totalWrongCount:$(e?.total_wrong_count,t.totalWrongCount),totalXp:$(e?.total_xp,t.totalXp),level:$(e?.level,t.level||1),coins:$(e?.coins,t.coins),diamonds:$(e?.diamonds,t.diamonds),monsterPoints:$(e?.monster_points,t.monsterPoints),defeatedMonsters:$(e?.defeated_monsters,t.defeatedMonsters)})}function Dp(e,t){return e?{autoPlayOnStudy:!!e.auto_play_on_study,autoPlayOnRecall:!!e.auto_play_on_recall,defaultAccent:e.default_accent===`uk`?`uk`:`us`,speechRate:[.8,.9,1,1.1].includes(Number(e.speech_rate))?Number(e.speech_rate):t.speechRate,repeatCount:Number(e.repeat_count)===2?2:1}:t}function Op(e){return[`study`,`recall`,`quiz`,`review`,`total`].includes(e)}async function kp(e,t){let n=Bc(),r=await n.from(`user_stats`).upsert(yp(e,t),{onConflict:`user_id`});if(r.error)throw r.error;let i=await n.from(`user_settings`).upsert(bp(e,t),{onConflict:`user_id`});if(i.error)throw i.error;await vp(`study_sessions`,e,xp(e,t.studySessions)),await vp(`checkins`,e,Sp(e,t.checkInHistory)),await vp(`rewards`,e,Cp(e,t.rewardHistory)),await vp(`achievements`,e,wp(e,t.checkedAchievementIds)),await vp(`inventory`,e,Tp(e,t.inventory))}async function Ap(e){let t=Bc(),[n,r,i,a,o,s,c]=await Promise.all([t.from(`user_stats`).select(`*`).eq(`user_id`,e).maybeSingle(),t.from(`user_settings`).select(`*`).eq(`user_id`,e).maybeSingle(),t.from(`study_sessions`).select(`*`).eq(`user_id`,e).order(`started_at`,{ascending:!1}),t.from(`checkins`).select(`*`).eq(`user_id`,e).order(`date`,{ascending:!1}),t.from(`rewards`).select(`*`).eq(`user_id`,e).order(`created_at`,{ascending:!1}),t.from(`achievements`).select(`*`).eq(`user_id`,e),t.from(`inventory`).select(`*`).eq(`user_id`,e)]);[n,r,i,a,o,s,c].forEach(e=>{if(e.error)throw e.error});let l=n.data?Ep(n.data):un(),u=(i.data||[]).map(e=>({id:e.id,date:String(e.started_at||e.created_at).slice(0,10),startedAt:e.started_at,endedAt:e.ended_at,durationSeconds:$(e.duration_seconds),mode:Op(e.mode)?e.mode:`study`,unitId:e.unit_id||void 0,wordCount:$(e.word_count),correctCount:e.correct_count==null?void 0:$(e.correct_count),wrongCount:e.wrong_count==null?void 0:$(e.wrong_count)})),d=(a.data||[]).filter(e=>e.type===`checkin`).map(e=>({date:e.date,checkedAt:e.created_at,streakDay:$(e.streak_day,1),rewardXp:$(e.reward_xp),rewardCoins:$(e.reward_coins)})),f=(o.data||[]).map(e=>({id:e.id,type:e.type,title:e.title,description:e.description||``,xp:$(e.xp),coins:$(e.coins),createdAt:e.created_at})),p=(s.data||[]).map(e=>e.achievement_id).filter(Boolean),m=(c.data||[]).map(e=>({itemId:e.item_id,count:$(e.count,1),obtainedAt:e.obtained_at}));return dn({...l,audioSettings:Dp(r.data,l.audioSettings),equippedTheme:r.data?.theme||l.equippedTheme,studySessions:u,checkInHistory:d,rewardHistory:f,checkedAchievementIds:p,inventory:m})}async function jp(e){let{count:t,error:n}=await Bc().from(`user_stats`).select(`id`,{count:`exact`,head:!0}).eq(`user_id`,e);if(n)throw n;return!!(t&&t>0)}var Mp=`reword-pending-sync-v1`,Np=`reword-sync-meta-v1`;function Pp(){return typeof window<`u`&&`localStorage`in window}function Fp(e){return`${e}-${typeof crypto<`u`&&`randomUUID`in crypto?crypto.randomUUID():`${Date.now()}-${Math.random().toString(36).slice(2,8)}`}`}function Ip(e,t){if(!Pp())return t;let n=window.localStorage.getItem(e);if(!n)return t;try{return JSON.parse(n)}catch{return t}}function Lp(e,t){Pp()&&window.localStorage.setItem(e,JSON.stringify(t))}function Rp(){return Ip(Mp,[])}function zp(e,t={}){let n=[...Rp(),{id:Fp(`sync`),type:e,payload:t,createdAt:new Date().toISOString(),retryCount:0}].slice(-100);return Lp(Mp,n),n}function Bp(){Lp(Mp,[])}function Vp(){return Ip(Np,{})}function Hp(e){Lp(Np,e)}function Up(){return{progress:nn(),userStats:fn(),exportedAt:new Date().toISOString(),schemaVersion:1}}function Wp(e){rn(e.progress),pn(e.userStats)}function Gp(e=Up()){let t=e.userStats;return Object.keys(e.progress).length>0||t.totalWordsLearned>0||t.totalWordsReviewed>0||t.totalQuizCount>0||t.totalStudyMinutes>0||t.rewardHistory.length>0||t.studySessions.length>0||t.checkInHistory.length>0}async function Kp(e){let[t,n]=await Promise.all([gp(e),jp(e)]);return t||n}async function qp(e,t=Up()){await mp(e,t.progress),await kp(e,t.userStats);let n={userId:e,lastSyncAt:new Date().toISOString(),lastMode:`upload`};return Hp(n),Bp(),n}async function Jp(e){let[t,n]=await Promise.all([hp(e),Ap(e)]);return{progress:t,userStats:n,exportedAt:new Date().toISOString(),schemaVersion:1}}function Yp(e){if(!e)return 0;let t=e.reviewHistory.reduce((e,t)=>Math.max(e,new Date(t.at).getTime()),0);return Math.max(t,e.lastReviewedAt?new Date(e.lastReviewedAt).getTime():0,e.nextReviewAt?new Date(e.nextReviewAt).getTime():0,e.firstLearnedAt?new Date(e.firstLearnedAt).getTime():0)}function Xp(e,t){let n=new Set([...Object.keys(e),...Object.keys(t)]),r={};return n.forEach(n=>{let i=e[n],a=t[n];if(!i){r[n]=a;return}if(!a){r[n]=i;return}let o=Yp(i)>=Yp(a)?i:a,s=[...i.reviewHistory,...a.reviewHistory].sort((e,t)=>new Date(e.at).getTime()-new Date(t.at).getTime()).filter((e,t,n)=>t===0||`${e.action}-${e.at}-${e.stage}`!=`${n[t-1].action}-${n[t-1].at}-${n[t-1].stage}`).slice(-200);r[n]={...o,reviewHistory:s}}),r}function Zp(e,t){let n=new Map;return e.forEach(e=>{n.set(t(e),e)}),Array.from(n.values())}function Qp(...e){let t=e.filter(e=>!!e).sort();return t.length?t[t.length-1]:null}function $p(e,t){let n={...t.dailyRewardCaps};return Object.entries(e.dailyRewardCaps).forEach(([e,t])=>{let r=n[e]||{xp:0,coins:0,monsterPoints:0};n[e]={xp:Math.max(r.xp,t.xp),coins:Math.max(r.coins,t.coins),monsterPoints:Math.max(r.monsterPoints,t.monsterPoints)}}),{...e,totalStudyDays:Math.max(e.totalStudyDays,t.totalStudyDays),currentStreak:Math.max(e.currentStreak,t.currentStreak),longestStreak:Math.max(e.longestStreak,t.longestStreak),signInStreak:Math.max(e.signInStreak,t.signInStreak),lastSignInDate:Qp(e.lastSignInDate,t.lastSignInDate),lastCheckInDate:Qp(e.lastCheckInDate,t.lastCheckInDate),totalStudyMinutes:Math.max(e.totalStudyMinutes,t.totalStudyMinutes),totalWordsLearned:Math.max(e.totalWordsLearned,t.totalWordsLearned),totalWordsReviewed:Math.max(e.totalWordsReviewed,t.totalWordsReviewed),totalQuizCount:Math.max(e.totalQuizCount,t.totalQuizCount),totalCorrectCount:Math.max(e.totalCorrectCount,t.totalCorrectCount),totalWrongCount:Math.max(e.totalWrongCount,t.totalWrongCount),totalXp:Math.max(e.totalXp,t.totalXp),level:Math.max(e.level,t.level),coins:Math.max(e.coins,t.coins),diamonds:Math.max(e.diamonds,t.diamonds),monsterPoints:Math.max(e.monsterPoints,t.monsterPoints),defeatedMonsters:Math.max(e.defeatedMonsters,t.defeatedMonsters),unlockedItems:Array.from(new Set([...e.unlockedItems,...t.unlockedItems])),checkedAchievementIds:Array.from(new Set([...e.checkedAchievementIds,...t.checkedAchievementIds])),defeatedWordIds:Array.from(new Set([...e.defeatedWordIds,...t.defeatedWordIds])),defeatedBossIds:Array.from(new Set([...e.defeatedBossIds,...t.defeatedBossIds])),dailyRewardCaps:n,checkInHistory:Zp([...e.checkInHistory,...t.checkInHistory],e=>e.date).sort((e,t)=>t.date.localeCompare(e.date)).slice(0,365),studySessions:Zp([...e.studySessions,...t.studySessions],e=>`${e.startedAt}-${e.mode}-${e.unitId||``}`).slice(0,500),rewardHistory:Zp([...e.rewardHistory,...t.rewardHistory],e=>`${e.createdAt}-${e.title}`).sort((e,t)=>t.createdAt.localeCompare(e.createdAt)).slice(0,200),inventory:Zp([...e.inventory,...t.inventory],e=>e.itemId).map(n=>{let r=e.inventory.find(e=>e.itemId===n.itemId),i=t.inventory.find(e=>e.itemId===n.itemId);return{...n,count:(r?.count||0)+(i?.count||0),obtainedAt:[r?.obtainedAt,i?.obtainedAt].filter(Boolean).sort()[0]||n.obtainedAt}}),audioSettings:t.audioSettings,soundSettings:t.soundSettings||e.soundSettings,equippedTheme:t.equippedTheme||e.equippedTheme,equippedAvatar:t.equippedAvatar||e.equippedAvatar}}function em(e,t){return{progress:Xp(e.progress,t.progress),userStats:$p(e.userStats,t.userStats),exportedAt:new Date().toISOString(),schemaVersion:1}}async function tm(e,t,n=Up()){try{if(t===`upload`)return await qp(e,n),n;let r=await Jp(e);if(t===`download`)return Wp(r),Hp({userId:e,lastSyncAt:new Date().toISOString(),lastMode:`download`}),Bp(),r;let i=em(n,r);return Wp(i),await qp(e,i),Hp({userId:e,lastSyncAt:new Date().toISOString(),lastMode:`merge`}),i}catch(e){throw Hp({...Vp(),lastError:e instanceof Error?e.message:`同步失败`}),e}}function nm(){let[e,t]=(0,y.useState)(()=>typeof navigator>`u`?!0:navigator.onLine);return(0,y.useEffect)(()=>{let e=()=>t(!0),n=()=>t(!1);return window.addEventListener(`online`,e),window.addEventListener(`offline`,n),()=>{window.removeEventListener(`online`,e),window.removeEventListener(`offline`,n)}},[]),{online:e}}var rm=Lc();function im({userId:e,progressMap:t,userStats:n,onApplySnapshot:r}){let{online:i}=nm(),a=(0,y.useRef)(Up()),o=(0,y.useRef)(),s=(0,y.useRef)(),c=Vp(),[l,u]=(0,y.useState)(()=>({configured:Fc,online:i,state:Fc?`idle`:`disabled`,message:Fc?`登录后可同步`:rm,lastSyncAt:c.lastSyncAt,pendingCount:Rp().length,needsFirstSyncChoice:!1}));(0,y.useEffect)(()=>{a.current={progress:t,userStats:n,exportedAt:new Date().toISOString(),schemaVersion:1}},[t,n]);let d=(0,y.useCallback)((e={})=>{let t=Vp();u(n=>({...n,configured:Fc,online:i,lastSyncAt:t.lastSyncAt,pendingCount:Rp().length,...e}))},[i]),f=(0,y.useCallback)(async t=>{if(!Fc)return d({state:`disabled`,message:rm}),null;if(!e)return d({state:`idle`,message:`登录后可同步`}),null;if(!i)return zp(`manual`,{mode:t}),d({state:`offline`,message:`离线中，已加入待同步队列`}),null;d({state:`syncing`,message:`同步中`});try{let n=await tm(e,t,a.current);return r(n),d({state:`success`,message:t===`upload`?`已上传本地进度`:t===`download`?`已恢复云端进度`:`已合并本地和云端进度`,needsFirstSyncChoice:!1}),n}catch(e){throw d({state:`error`,message:e instanceof Error?e.message:`同步失败`}),e}},[i,r,d,e]),p=(0,y.useCallback)((t,n={})=>{if(!Fc){d({state:`disabled`,message:rm});return}if(!e){d({state:`idle`,message:`游客本地模式`});return}if(!i){zp(t,n),d({state:`offline`,message:`离线中，已先保存到本地`});return}window.clearTimeout(o.current),o.current=window.setTimeout(()=>{f(`upload`).catch(()=>void 0)},900)},[i,d,f,e]);return(0,y.useEffect)(()=>{d({state:Fc?i?l.state===`offline`?`idle`:l.state:`offline`:`disabled`,message:Fc?i?l.message:`离线模式`:rm})},[i]),(0,y.useEffect)(()=>{!Fc||!e||!i||Rp().length&&f(`upload`).catch(()=>void 0)},[i,f,e]),(0,y.useEffect)(()=>{if(!Fc||!e||s.current===e)return;s.current=e;let t=Vp();if(t.userId===e&&t.lastSyncAt){d({state:`success`,message:`已连接云同步`,needsFirstSyncChoice:!1});return}let n=!0;return Kp(e).then(async e=>{if(!n)return;let t=Gp(a.current);t&&e?d({state:`idle`,message:`请选择首次同步方式`,needsFirstSyncChoice:!0}):t&&!e?await f(`upload`):!t&&e?await f(`download`):d({state:`success`,message:`已连接云同步`,needsFirstSyncChoice:!1})}).catch(e=>{n&&d({state:`error`,message:e instanceof Error?e.message:`读取云端数据失败`})}),()=>{n=!1}},[d,f,e]),(0,y.useEffect)(()=>()=>window.clearTimeout(o.current),[]),{...l,syncNow:f,queueLocalChange:p,refreshStatus:d}}var am=new Set([`click`,`success`,`error`,`reward`,`toggle`,`correct`,`wrong`,`wordLearned`,`monsterDefeated`,`levelUp`]);function om(e){(0,y.useEffect)(()=>{let t=t=>{let n=(t.target instanceof Element?t.target:null)?.closest(`button, a`);if(!n||n.getAttribute(`aria-disabled`)===`true`||`disabled`in n&&n.disabled)return;let r=n.dataset.sound;if(r!==`none`){if(r&&am.has(r)){Vd(r,e);return}Hd(e)}};return document.addEventListener(`pointerdown`,t,{capture:!0}),()=>document.removeEventListener(`pointerdown`,t,{capture:!0})},[e.enabled,e.volume])}function sm(e){let t=new Blob([e],{type:`application/json`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`reword-progress-${new Date().toISOString().slice(0,10)}.json`,r.click(),URL.revokeObjectURL(n)}function cm(e,t,n=6e4){return e.lastReviewedAt?t.getTime()-new Date(e.lastReviewedAt).getTime()<n:!1}function lm(e,t){return t.rewardHistory[0]&&t.rewardHistory[0]?.id!==e.rewardHistory[0]?.id}function um(){let[e,t]=(0,y.useState)({name:`dashboard`}),[n,r]=(0,y.useState)(()=>nn()),[i,a]=(0,y.useState)(()=>fn()),[o,s]=(0,y.useState)(),[c,l]=(0,y.useState)(!1),[u,d]=cl(`motivationSettings`,Ln),[f,p]=cl(`motivation.lastShownDate`,null),[m,h]=(0,y.useState)(!1),[g,_]=(0,y.useState)(null),v=(0,y.useRef)(i.level),b=(0,y.useRef)(!1),x=ap();om(i.soundSettings),(0,y.useEffect)(()=>{let e=v.current;if(v.current=i.level,!b.current||i.level<=e)return;b.current=!1;let t=window.setTimeout(()=>Jd(i.soundSettings),140);return()=>window.clearTimeout(t)},[i.level,i.soundSettings.enabled,i.soundSettings.volume]);let S=(0,y.useCallback)(e=>{r(e.progress),a(e.userStats)},[]),C=(0,y.useCallback)(e=>{sn(e.unitId,e.id)},[]),w=im({userId:x.user?.id,progressMap:n,userStats:i,onApplySnapshot:S}),T=(0,y.useMemo)(()=>np.find(t=>t.id===e.unitId)||np[0],[e.unitId]),E=(0,y.useMemo)(()=>{if(e.name===`quiz`&&e.quizMode===`wrong`){let e=Dn(rp,n);return e.length?e:rp}return e.unitId?T.words:rp},[n,e.name,e.quizMode,e.unitId,T.words]);(0,y.useEffect)(()=>{x.user&&w.needsFirstSyncChoice&&e.name!==`sync`&&t({name:`sync`})},[x.user,w.needsFirstSyncChoice,e.name]),(0,y.useEffect)(()=>{(e.name===`study`||e.name===`recall`||e.name===`quiz`)&&window.scrollTo({top:0})},[e.name,e.unitId]),(0,y.useEffect)(()=>{if(!o)return;let e=window.setTimeout(()=>s(void 0),2600);return()=>window.clearTimeout(e)},[o]),(0,y.useEffect)(()=>{let e=()=>l(!0);return window.addEventListener(`reword:update-ready`,e),()=>window.removeEventListener(`reword:update-ready`,e)},[]),(0,y.useEffect)(()=>{let t=Lt();!u.enabled||!u.launchModal||e.name!==`dashboard`||f===t||h(!0)},[f,u.enabled,u.launchModal,e.name]);let ee=async()=>{let e=await navigator.serviceWorker?.getRegistration();if(e?.waiting){e.waiting.postMessage({type:`SKIP_WAITING`});return}window.location.reload()},te=async()=>{if(window.confirm(`只清理 PWA 缓存和旧版本文件，不会删除学习进度。继续吗？`)){if(`caches`in window){let e=await caches.keys();await Promise.all(e.filter(e=>e.startsWith(`reword-cache-`)).map(e=>caches.delete(e)))}if(`serviceWorker`in navigator){let e=await navigator.serviceWorker.getRegistrations();await Promise.all(e.map(e=>e.unregister()))}window.location.reload()}},ne=(e,t,r=n)=>{let i=Qc(t,rp,r);return pn(i),w.queueLocalChange(`stats`,{source:`stats-update`}),lm(e,i)&&s(i.rewardHistory[0]),i.level>e.level&&(b.current=!0),i},D=(e,t=n)=>{a(n=>ne(n,e(n),t))},re=(e,t,n)=>{r(r=>{let i=r[e]||$t(e),o=t(i),s={...r,[e]:o};return rn(s),w.queueLocalChange(`progress`,{wordId:e}),n&&a(e=>ne(e,n(e,i,o,s),s)),s})},ie=(e,t)=>{if(Cn(t)&&!i.defeatedWordIds.includes(e)){Kd(i.soundSettings);return}Ud(i.soundSettings)},ae=e=>{if(n[e]?.learned||n[e]?.firstLearnedAt){Hd(i.soundSettings);return}Wd(i.soundSettings),re(e,e=>vn(e),(e,t)=>t.learned||t.firstLearnedAt?e:M({...e,totalWordsLearned:e.totalWordsLearned+1},{type:`study`,title:`新词入库`,description:`完成 1 个新单词初学`,xp:2,coins:1,monsterPoints:1}))},O=(e,t)=>{let r=new Date,a=n[e]||$t(e);t===`known`?ie(e,yn(a,r)):t===`fuzzy`?Yd(i.soundSettings):Gd(i.soundSettings),re(e,e=>t===`known`?yn(e,r):t===`fuzzy`?bn(e,r):xn(e,r),(n,i,a)=>{if(cm(i,r))return n;let o={...n,totalWordsReviewed:n.totalWordsReviewed+1};return t===`known`?o=M(o,{type:`review`,title:`复习命中`,description:`理解模式记住 1 个单词`,xp:4,coins:2,monsterPoints:3}):t===`fuzzy`&&(o=M(o,{type:`review`,title:`复习推进`,description:`标记 1 个不熟词，进入加固周期`,xp:1,monsterPoints:1})),t===`known`&&Cn(a)&&!o.defeatedWordIds.includes(e)&&(o=M({...o,defeatedWordIds:[...o.defeatedWordIds,e],defeatedMonsters:o.defeatedMonsters+1},{type:`monster`,title:`击败单词小怪`,description:`一个单词进入掌握状态`,xp:10,coins:5,monsterPoints:10})),o})},oe=(e,t)=>{let r=new Date,a=n[e]||$t(e);t?ie(e,yn({...a,correctCount:a.correctCount+1},r)):Gd(i.soundSettings),re(e,e=>t?en(yn({...e,correctCount:e.correctCount+1},r),`quiz-correct`,r):en(xn({...e,wrongCount:e.wrongCount+1},r),`quiz-wrong`,r),(n,i,a)=>{let o=cm(i,r),s={...n,totalQuizCount:n.totalQuizCount+1,totalCorrectCount:n.totalCorrectCount+ +!!t,totalWrongCount:n.totalWrongCount+ +!t};return!o&&t&&(s=M(s,{type:`quiz`,title:`测试答对`,description:`测验中答对 1 题`,xp:3,coins:1,monsterPoints:2})),t&&Cn(a)&&!s.defeatedWordIds.includes(e)&&(s=M({...s,defeatedWordIds:[...s.defeatedWordIds,e],defeatedMonsters:s.defeatedMonsters+1},{type:`monster`,title:`击败单词小怪`,description:`测试命中让单词进入掌握状态`,xp:10,coins:5,monsterPoints:10})),s})},se=e=>{try{r(ln(e)),a(fn()),w.queueLocalChange(`manual`,{source:`import`})}catch(e){window.alert(e instanceof Error?e.message:`导入失败`)}},ce=()=>{window.confirm(`确定清空所有学习进度、金币和打卡记录吗？`)&&(an(),r({}),a(fn()),w.queueLocalChange(`manual`,{source:`reset`}))},le=(0,y.useCallback)(e=>{a(t=>{let n=vl(t).todaySeconds,r=_l(t,e);if(r===t)return t;let i=vl(r).todaySeconds;[{minutes:15,xp:20,coins:8,monsterPoints:8},{minutes:30,xp:35,coins:12,monsterPoints:12},{minutes:60,xp:80,coins:25,monsterPoints:25}].forEach(e=>{n<e.minutes*60&&i>=e.minutes*60&&(r=M(r,{type:`study`,title:`专注 ${e.minutes} 分钟`,description:`今日学习时长达标`,xp:e.xp,coins:e.coins,monsterPoints:e.monsterPoints}))});let a=Qc(r,rp,nn());return pn(a),w.queueLocalChange(`session`,{sessionId:e.id}),lm(t,a)&&s(a.rewardHistory[0]),a.level>t.level&&(b.current=!0),a})},[w.queueLocalChange]),k=()=>{qd(i.soundSettings),D(e=>Pn(e))},ue=()=>{if(!Nn(rp,n,i)){Gd(i.soundSettings),window.alert(`今日任务还没达成，先学几个新词、复习或完成一次测试。`);return}qd(i.soundSettings);let e=jn(rp,n,i),t=Fn(i),r=t.checkInHistory[0];r&&u.enabled&&u.checkinMotivation&&_({streak:t.currentStreak,studyMinutes:e.studyMinutes,wordCount:e.newWords+e.reviewedWords,xp:r.rewardXp,coins:r.rewardCoins}),D(e=>Fn(e))},de=e=>{ef(i,e)===i?Gd(i.soundSettings):qd(i.soundSettings),a(t=>{let n=ef(t,e);return n===t?(window.alert(`金币/钻石不足，或已经拥有这个物品。`),t):ne(t,n)})},fe=e=>{let t=np.find(t=>t.id===e);if(!t)return;let r=Ul(t,n,i);if(!r.eligible||r.defeated){Gd(i.soundSettings),window.alert(`Boss 还不能挑战：需要本单元学习达到 80%，并且没有到期复习。`);return}Kd(i.soundSettings),D(t=>M({...t,defeatedBossIds:[...t.defeatedBossIds,e],defeatedMonsters:t.defeatedMonsters+1},{type:`monster`,title:`击败 ${r.name}`,description:`完成一个单元 Boss 挑战`,xp:100,coins:50,diamonds:1,monsterPoints:60,bypassDailyCap:!0}))},pe=e=>{D(t=>({...t,audioSettings:e}))},me=e=>{D(t=>({...t,soundSettings:e}))},he=e=>{d(e)},ge=()=>{p(Lt()),h(!1)},_e=()=>{ge(),ve()},ve=()=>{let e=on();if(e&&rp.some(t=>t.id===e.wordId)){let n=np.find(t=>t.id===e.unitId)||np.find(t=>t.words.some(t=>t.id===e.wordId));if(n){t({name:`study`,unitId:n.id,wordId:e.wordId});return}}t({name:`study`,unitId:(np.find(e=>e.words.some(e=>!n[e.id]?.learned&&!n[e.id]?.firstLearnedAt))||np[0]).id})},ye=e=>t({name:e}),be=e.name===`study`||e.name===`quiz`||e.name===`total`?e.name:e.name===`recall`?e.dueOnly?`review`:`recall`:void 0;return(0,A.jsxs)(Pt,{current:e.name,onNavigate:ye,user:x.user,profile:x.profile,children:[o?(0,A.jsx)(`div`,{className:`fixed left-1/2 top-20 z-50 w-[min(92vw,360px)] -translate-x-1/2`,children:(0,A.jsx)(Ft,{reward:o})}):null,c?(0,A.jsxs)(`div`,{className:`fixed inset-x-3 top-16 z-50 mx-auto flex max-w-md items-center justify-between gap-3 rounded-lg border border-indigo-200 bg-white px-3 py-2 text-sm shadow-soft md:top-4`,children:[(0,A.jsx)(`span`,{className:`font-semibold text-ink`,children:`发现新版本`}),(0,A.jsxs)(`div`,{className:`flex shrink-0 gap-2`,children:[(0,A.jsx)(`button`,{type:`button`,onClick:()=>l(!1),className:`rounded-md border border-slate-200 px-2 py-1 text-xs font-semibold text-slate-500`,children:`稍后`}),(0,A.jsx)(`button`,{type:`button`,onClick:te,className:`rounded-md border border-sky-200 px-2 py-1 text-xs font-semibold text-harbor`,children:`清缓存`}),(0,A.jsx)(`button`,{type:`button`,onClick:ee,className:`rounded-md bg-harbor px-2 py-1 text-xs font-semibold text-white`,children:`刷新`})]})]}):null,e.name===`login`?(0,A.jsx)(Bl,{configured:x.configured,loading:x.loading,error:x.error,onLogin:async(e,t)=>{await x.login(e,t)},onSuccess:()=>t({name:`sync`}),onRegister:()=>t({name:`register`}),onGuest:()=>t({name:`dashboard`})}):e.name===`register`?(0,A.jsx)(gd,{configured:x.configured,loading:x.loading,error:x.error,onRegister:async(e,t,n)=>await x.register(e,t,n),onSuccess:()=>t({name:x.user?`sync`:`login`}),onLogin:()=>t({name:`login`})}):e.name===`sync`?(0,A.jsx)(uf,{configured:w.configured,online:w.online,user:x.user,state:w.state,message:w.message,lastSyncAt:w.lastSyncAt,pendingCount:w.pendingCount,onSync:async e=>{await w.syncNow(e),t({name:`account`})},onSkip:()=>t({name:`dashboard`}),onLogin:()=>t({name:`login`})}):e.name===`account`?(0,A.jsx)(Jc,{configured:w.configured,online:w.online,user:x.user,profile:x.profile,state:w.state,message:w.message,lastSyncAt:w.lastSyncAt,pendingCount:w.pendingCount,onLogin:()=>t({name:`login`}),onRegister:()=>t({name:`register`}),onSync:w.syncNow,onLogout:async()=>{await x.logout(),t({name:`dashboard`})},onExport:()=>sm(cn()),onImport:se}):e.name===`dashboard`?(0,A.jsx)(Tl,{units:np,words:rp,progressMap:n,stats:i,motivationSettings:u,onSignIn:k,onContinueStudy:ve,onNavigateReview:()=>t({name:`review`}),onNavigateWrongQuiz:()=>t({name:`quiz`,quizMode:`wrong`}),onNavigateUnits:()=>t({name:`units`}),onNavigateCheckIn:()=>t({name:`checkin`}),onNavigateMonster:()=>t({name:`monster`}),onNavigateShop:()=>t({name:`shop`}),onNavigateStatistics:()=>t({name:`statistics`}),onNavigateLogin:()=>t({name:`login`}),onNavigateAccount:()=>t({name:`account`}),syncStatus:{configured:w.configured,online:w.online,state:w.state,message:x.user?w.message:w.configured?`游客本地模式`:w.message,lastSyncAt:w.lastSyncAt,pendingCount:w.pendingCount,isLoggedIn:!!x.user}}):e.name===`units`?(0,A.jsx)(mf,{units:np,progressMap:n,onOpenUnit:e=>t({name:`unitDetail`,unitId:e})}):e.name===`unitDetail`?(0,A.jsx)(ff,{unit:T,progressMap:n,onStudy:()=>t({name:`study`,unitId:T.id}),onRecall:()=>t({name:`recall`,unitId:T.id}),onQuiz:()=>t({name:`quiz`,unitId:T.id})}):e.name===`study`?(0,A.jsx)(lf,{unit:e.unitId?T:void 0,words:E,progressMap:n,audioSettings:i.audioSettings,initialWordId:e.wordId,onLearn:ae,onStudyPositionChange:C}):e.name===`recall`?(0,A.jsx)(dd,{title:e.unitId?`${T.name} 理解功能`:`总理解功能`,words:E,progressMap:n,audioSettings:i.audioSettings,initialDueOnly:e.dueOnly,onGrade:O,onBackToStudy:()=>t({name:`study`,unitId:e.unitId})}):e.name===`quiz`?(0,A.jsx)(cd,{title:e.quizMode===`wrong`?`错题强化测试`:e.unitId?`${T.name} 测试功能`:`总测试功能`,words:E,allWords:rp,progressMap:n,audioSettings:i.audioSettings,motivationSettings:u,onAnswer:oe}):e.name===`total`?(0,A.jsx)(df,{words:rp,progressMap:n,audioSettings:i.audioSettings,onStudy:()=>t({name:`study`}),onRecall:()=>t({name:`recall`}),onQuiz:()=>t({name:`quiz`})}):e.name===`review`?(0,A.jsx)(_d,{words:rp,progressMap:n,audioSettings:i.audioSettings,onStartReview:()=>t({name:`recall`,dueOnly:!0}),onOpenUnits:()=>t({name:`units`})}):e.name===`checkin`?(0,A.jsx)(nl,{words:rp,progressMap:n,stats:i,onSignIn:k,onCheckIn:ue}):e.name===`rewards`?(0,A.jsx)(vd,{stats:i}):e.name===`shop`?(0,A.jsx)(nf,{stats:i,onExchange:de}):e.name===`monster`?(0,A.jsx)(lu,{words:rp,units:np,progressMap:n,stats:i,onChallengeBoss:fe}):e.name===`statistics`?(0,A.jsx)(af,{stats:i,words:rp,progressMap:n}):e.name===`achievements`?(0,A.jsx)($c,{stats:i,words:rp,progressMap:n}):e.name===`settings`?(0,A.jsx)(Zd,{stats:i,onUpdateAudio:pe,onUpdateSound:me,motivationSettings:u,onUpdateMotivation:he,syncStatus:{online:w.online,state:w.state,message:w.message,lastSyncAt:w.lastSyncAt,pendingCount:w.pendingCount,isLoggedIn:!!x.user}}):(0,A.jsx)(uu,{words:rp,progressMap:n,stats:i,onNavigateCheckIn:()=>t({name:`checkin`}),onNavigateRewards:()=>t({name:`rewards`}),onNavigateShop:()=>t({name:`shop`}),onNavigateStatistics:()=>t({name:`statistics`}),onNavigateAchievements:()=>t({name:`achievements`}),onNavigateSettings:()=>t({name:`settings`}),onNavigateAccount:()=>t({name:`account`}),syncStatus:{configured:w.configured,online:w.online,state:w.state,message:x.user?w.message:w.configured?`游客本地模式`:w.message,lastSyncAt:w.lastSyncAt,pendingCount:w.pendingCount},onExport:()=>sm(cn()),onImport:se,onClearCache:te,onReset:ce}),(0,A.jsx)(tr,{open:m,settings:u,onStart:_e,onClose:ge}),(0,A.jsx)(Jn,{open:!!g,summary:g,settings:u,onClose:()=>_(null)}),(0,A.jsx)(gf,{mode:be,unitId:e.unitId,wordCount:E.length,onComplete:le})]})}St.createRoot(document.getElementById(`root`)).render((0,A.jsx)(y.StrictMode,{children:(0,A.jsx)(um,{})})),`serviceWorker`in navigator&&window.addEventListener(`load`,()=>{let e=!1,t=()=>window.dispatchEvent(new CustomEvent(`reword:update-ready`));navigator.serviceWorker.addEventListener(`controllerchange`,()=>{e||(e=!0,window.location.reload())}),navigator.serviceWorker.register(`/reword/sw.js`).then(e=>{e.update(),e.waiting&&t(),e.addEventListener(`updatefound`,()=>{let n=e.installing;n?.addEventListener(`statechange`,()=>{n.state===`installed`&&navigator.serviceWorker.controller&&t()})})}).catch(()=>void 0)});