...
export function MonsterPage({ words, units, progressMap, stats, onChallengeBoss }: { words: WordEntry[]; units: WordUnit[]; progressMap: ProgressMap; stats: UserStats; onChallengeBoss: (unitId: string) => void }) {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-lg border border-violet-200 bg-white p-5 shadow-soft">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-lg bg-violet-50 p-4 text-center"><div className="text-2xl font-semibold">{summary.monsterPoints}</div><div className="text-sm text-slate-500">打怪积分</div></div>
          <div className="rounded-lg bg-violet-50 p-4 text-center"><div className="text-2xl font-semibold">{summary.defeatedWords}</div><div className="text-sm text-slate-500">击败小怪</div></div>
          <div className="rounded-lg bg-violet-50 p-4 text-center"><div className="text-2xl font-semibold">{summary.defeatedBosses}</div><div className="text-sm text-slate-500">击败 Boss</div></div>
        </div>
      </section>
